...
The 11 binary classifiers reported in Section~\ref{sec:results} were trained and evaluated independently. In this section we describe how these classifiers can be composed into a hierarchical screening cascade that mirrors the clinical decision sequence a primary care clinician might follow when evaluating a respiratory complaint, and we illustrate the composition on three test-set patients selected as worked examples.

The cascade begins with B00 (acute respiratory versus acute non-respiratory illness). A patient classified as respiratory is passed to B02 (infection versus non-infectious mimic), which separates active infection from allergic, structural, or symptom-only presentations. A patient classified as having an active infection is then passed to B01 (acute infection versus chronic respiratory disease management), which separates acute illness from chronic management visits. A patient classified as acute infection is passed to B03 (lower versus upper respiratory infection). On the lower respiratory branch, B04 (pneumonia versus acute bronchitis) provides the pneumonia-versus-airway distinction. On the upper respiratory branch, B06 (acute sinusitis versus other URI) and B07 (Group A streptococcal versus other pharyngitis) are applied in sequence. Three additional classifiers, B08 (influenza), B09 (COVID-19), and B10 (bacterial versus viral), are applied in parallel as an etiology overlay to any patient routed through the acute infection branch, since these contrasts cut across anatomical localization. At each cascade node the per-binary decision threshold from Section~\ref{sec:results-threshold} is used; thresholds are not jointly optimized for cascade traversal.

Figure~\ref{fig_case_studies} illustrates the cascade on three test-set patients. The cases were selected as worked examples that traverse the cascade end to end without misrouting; they are not representative of average cascade behavior, and aggregate cascade-traversal statistics on the test set are provided in the supplementary material. Case C1 (community-acquired pneumonia, ICD-10 J18.0) traces a full lower respiratory pathway: B00 positive (acute respiratory), B02 positive (infection), B01 positive (acute), B03 positive (lower respiratory), B04 positive (pneumonia), terminating at pneumonia as the cascade-routed classification. The etiology overlay returns a positive call for bacterial (B10), which is consistent with the typical aetiology of community-acquired pneumonia, while influenza (B08) and COVID-19 (B09) are negative. Case C2 (Group A streptococcal pharyngitis, ICD-10 J02.0) traces a full upper respiratory pathway: B00 positive, B02 positive, B01 positive, B03 negative (upper rather than lower), B07 positive, terminating at GAS pharyngitis. The etiology overlay is positive for bacterial (B10) and negative for influenza and COVID-19, consistent with the underlying diagnosis. Case C4 (lumbosacral radiculopathy, ICD-10 M54.42) exits at the gate task: B00 returns a low probability (0.070) well below the threshold of 0.491, and the cascade terminates at the non-respiratory branch on a single step. The etiology overlay does not apply for cases that exit at the gate task, since the patient is not routed through any acute respiratory infection branch.

These three cases illustrate three distinct cascade behaviors: deep traversal through the lower respiratory pathway, deep traversal through the upper respiratory pathway, and immediate rejection at the gate task for a non-respiratory presentation. In each, the per-step decisions agree with the underlying ICD-10 ground truth at the nodes where ground truth is defined. The cases are not selected to characterize cascade performance overall; they are presented to make concrete what the cascade does when its component classifiers agree along a clinically meaningful path. We treat the cascade itself as a proof-of-concept composition rather than a deployment-ready triage system, and the gap between independent-task performance reported in Section~\ref{sec:results} and any composed cascade performance is one of the open questions returned to in the Discussion.
